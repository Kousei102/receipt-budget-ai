...
export function receiptsToCsv(receipts: StoredReceipt[]): string {
  const rows: string[][] = [["購入日", "店名", "品目", "カテゴリ", "金額", "種別"]];
  for (const r of receipts) {
    const source = SOURCE_LABELS[r.source];
    if (r.items.length === 0) {
      rows.push([r.date, r.store, "", "", String(r.total), source]);
      continue;
    }
    for (const it of r.items) {
      rows.push([r.date, r.store, it.name, it.category, String(it.price), source]);
    }
  }
