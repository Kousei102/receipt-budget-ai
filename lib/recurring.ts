 ...
 * 各定義について max(startMonth, lastPostedMonth の翌月) 〜 min(nowMonth, endMonth) の月を列挙し、
 * まだ存在しない月のレコードを作る。当月分は支払日未到来でも計上する（予定支出として集計に載せる）。
 *
 * 冪等性の二段構え:
...
export function materializeRecurring(
  ...
  const nextDefs = defs.map((def) => {
    ...
    const end =
      def.endMonth && def.endMonth < nowMonth ? def.endMonth : nowMonth;
    ...
    return { ...def, lastPostedMonth: end };
  });
