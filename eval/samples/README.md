...
```json
{
  ...
  "date": "2026-07-01",
  "items": [
    { "name": "おにぎり", "price": 150, "category": "食費" },
...
- `category` は**品目ごと**に付ける（現在の eval は品目単位でカテゴリ一致を採点する）。
  値は `lib/schema.ts` の `CATEGORIES`（食費 / 日用品 / 外食 / 交通 / 娯楽 / その他）から選ぶ。
- `price` は「2個で¥196」のようなまとめ行はまとめた金額を、値引き行は品目化せず `total` にのみ反映する。
- 日付が架空（例: `20XX年`）で実年が無いレシートは、アプリの仕様に合わせて「その年＝今年」で埋める。
