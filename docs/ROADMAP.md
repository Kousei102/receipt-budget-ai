...
Nokori の改善計画。優先度順（A → B → C）。完了したら `[x]` にチェックを入れる。
...
- [x] **D-6. クレジットカード明細の取り込み（レシート以外の支出形態・その4）**
  - なぜ: クレカ支出はレシートを撮り忘れがちで、カード会社のWeb明細・アプリ画面から一括計上できると記録漏れが減る。また店頭レシートと明細の二重計上が新たな課題になる。
  - 何を: 抽出プロンプトに3種目の画像タイプ「クレカ利用明細」を追加（利用1件=1レコード、store=加盟店名を全角化、date=利用日、items は1品目で店名から業種推定。返金・キャッシング・繰越行は除外、年会費・手数料は含める）。Tool スキーマのトップレベルに `imageKind`（receipt / payment_app / card_statement）を追加し、クライアントが `card_statement` のとき `source: "card"` を付与（バッジ「カード明細」・CSV種別に自動反映）。二重計上対策として `lib/duplicate.ts` に緩い一致（NFKC 正規化＋法人格除去の店名部分一致・日付±3日・金額±1円）を追加。誤検知を抑えるため「card vs 非card」のペアに限定して適用し、候補は既存の重複確認UI（追加する/✕）でユーザーが最終判断する。
