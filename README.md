...
> **🔗 デモ**: https://receipt-budget-ai.vercel.app/
...
app/
  ...
  api/extract/route.ts   画像を受け Claude を呼ぶ API（ユーザーキーを中継 or env にフォールバック）
lib/
...
APIキーは **BYOK（Bring Your Own Key）優先**です。ユーザーが画面下の「🔑 APIキー設定」で登録したキーをブラウザの localStorage に保存し、読み取り時にだけヘッダーでサーバーに中継して Claude を呼びます（サーバー側で保存・記録はしません）。キー未設定の場合は、サーバーの環境変数 `ANTHROPIC_API_KEY` にフォールバックします。
...
# 2. APIキーを設定（どちらか一方でOK）
#   a) アプリ起動後、画面下の「🔑 APIキー設定」に自分のキーを登録（BYOK）
#   b) 開発者向け: .env.local に ANTHROPIC_API_KEY を設定（eval の実行には必須）
...
3. （任意）環境変数に `ANTHROPIC_API_KEY` を設定 — 設定しなければ訪問者は「🔑 APIキー設定」から自分のキーを使う（BYOK専用運用）
